...
void PLAYER::DrawFrame ()
{
	...
	wchar_t tmp[50];
	_stprintf_s ( tmp, L"最好记录： %.3f 块/秒", g[0] );
...
void wphb ()
{
	ofstream OutFile ( "phb.txt" );
	sprintf_s ( temp, "no %s:%.3f;%s:%.3f;%s:%.3f;", n[0], g[0], n[1], g[1], n[2], g[2] );
	OutFile << temp;
	OutFile.close ();
}
...
{
	//读取文件
	ifstream readFile ( "phb.txt" );
	readFile >> temp;
	readFile.getline ( temp, 1024, 0 );
	readFile.close ();
	int i, j = 0, k = 0;
	//处理获得的文本
	sscanf_s ( temp, " %[^:]:%f;%[^:]:%f;%[^:]:%f;", n[0], 40, &g[0], n[1], 40, &g[1], n[2], 40, &g[2] );
...
void dphb ()
{
	...
	for ( int i = 0; i < 3; i++ )
	{
		...
		sprintf_s ( cj[i], "%s:%.3f块/s", n[i], g[i] );
		RECT a = { 320 * px + 20,70 + i * 30,320 * px + 20 + 200,100 + i * 30 };
		drawtext ( char2wchar ( cj[i] ), &a, DT_CENTER | DT_VCENTER | DT_SINGLELINE );
	}
}
//char转wchar_t
