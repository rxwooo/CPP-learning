...
char *FILENAME = "FileSys.dat";     // 文件系统名
...
void startsys()
{
    ...
    printf("开始读取文件...");
    FILE *file;
    if ((file = fopen(FILENAME, "r")) != NULL)//打开返回文件标识符，打不开返回NULL
    {
        fread(buffer, SIZE, 1, file); //将二进制文件读取到缓冲区
        fclose(file);
        if (memcmp(buffer, "10101010", 8) == 0)//比较魔数是否一致
        {
            memcpy(v_addr0, buffer, SIZE);//从缓冲区复制到磁盘空间上
    ...
    //	⑨ 将当前目录设置为根目录。
    fcb *root;
    root = (fcb *)(v_addr0 + 5 * BLOCKSIZE); //第5块
    ...
    openfilelist[0].topenfile = 1;//已被占据

    //初始化全局变量
    // startp指向数据区的开头
    startp = ((block0 *)v_addr0)->startblock;//都是全局变量
    currfd = 0;
    return;
}
void my_format()
{
    ...
    fat *fat2 = (fat *)(v_addr0 + BLOCKSIZE * 3);//fat占2个块
    ...
    root->metadata = 0;//0表示目录文件

    time_t rawTime = time(NULL); //以秒为单位
    ...
    root->date = (time->tm_year - 100) * 512 + (time->tm_mon + 1) * 32 + (time->tm_mday);//不知道
    root->first = 5;//起始盘块号
    root->free = 1;//已分配
    root->length = 2 * sizeof(fcb);//初始两个文件..和.

    // root2 指向根目录区的第二个fcb,即特殊目录项..,因为根目录区没有上级目录,所以指向自己
    fcb *root2 = root + 1;//后一个
    memcpy(root2, root, sizeof(fcb));
    strcpy(root2->filename, "..");

    for (int i = 2; i < int(BLOCKSIZE / sizeof(fcb)); i++)//有几个fcb
    {
        root2++;
        strcpy(root2->filename, "");
        root2->free = 0;
...
void my_cd(char *dirname)
{
    ...
    //          这样就算是打开一个目录
    if (openfilelist[currfd].metadata == 1)//1表示数据文件
    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    strcpy(openfilelist[fd].dir,
                           (char *)(string(openfilelist[currfd].dir) + string(dirname) + string("\\")).c_str());//将string转换成c中的char
...
void my_mkdir(char *dirname)
{
    ...
    {
        if (strncmp(dirname, fcbPtr[i].filename, 8) == 0 && fcbPtr->metadata == 0)
        {
            cout << "目录名已经存在!" << endl;
            return;
        }
...
int my_create(char *filename)
{
    ...
    for (i = 0; i < int(openfilelist[currfd].length / sizeof(fcb)); i++, fcbPtr++)
    {
        if (strncmp(fcbPtr->filename, filename, 8) == 0 && strncmp(fcbPtr->exname, exname, 3) == 0)
        {
            cout << "已有同名文件存在!" << endl;
            return -1;
        }
